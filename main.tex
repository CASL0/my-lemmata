...
\begin{lemma}[1.11]
  ...
  このとき、
  \[\forall n \in \mathbb{N}: \quad a_n \leq b_n \Rightarrow a \leq b\]

  \begin{proof}
    $a > b$と仮定する。

    \[\varepsilon := \dfrac{a-b}{2} > 0\]

    $\lim_{n \to \infty} a_n = a$より、
    \[\exists n_1 \in \mathbb{N}: \quad n \geq n_1 \Rightarrow
    |a_n - a| < \varepsilon\]
    したがって、
    \[a_n > a - \varepsilon = \dfrac{a+b}{2}\]

    $\lim_{n \to \infty} b_n = b$より、
    \[\exists n_2 \in \mathbb{N}: \quad n \geq n_2 \Rightarrow
    |b_n - b| < \varepsilon\]
    したがって、
    \[b_n < b + \varepsilon = \dfrac{a+b}{2}\]

    $n_0 := \max(n_1, n_2)$とすると、
    \[n \geq n_0 \Rightarrow a_n > \dfrac{a+b}{2} > b_n\]

    これは仮定$a_n \leq b_n$に矛盾する。

    \[\therefore \quad a \leq b\]
  \end{proof}
\end{lemma}
...
\begin{lemma}[1.36]
  ...
  \begin{proof}
    ...
    が成り立つ。
    $n_0 := 2\max(n_1, n_2) - 1$とすると、
    $n \geq n_0 \Rightarrow |c_n -a| < \varepsilon$

    $\because$ $c_{2k-1} = a_k$、$c_{2k} = b_k$（$k \in \mathbb{N}$）の対応より、
    $n \geq n_0$ が奇数 $n = 2k-1$ の時は $k \geq n_1$ より
    $|c_n - a| = |a_k - a| < \varepsilon$、
    $n$ が偶数 $n = 2k$ の時は $k \geq n_2$ より $|c_n - a| = |b_k - a| < \varepsilon$
...
\begin{lemma}[1.6]
  ...
  \begin{proof}
    \begin{align*}
      \frac{\partial f}{\partial x_k} &= \frac{\partial}{\partial
      x_k}\sum_{i = 1}^{n}\sum_{j = 1}^{n} a_{ij}x_ix_j \\
      &= \sum_{i = 1}^{n}\sum_{j = 1}^{n} \frac{\partial}{\partial
      x_k}a_{ij}x_ix_j \\
      \text{(積の微分より)} &= \sum_{i = 1}^{n}\sum_{j = 1}^{n} \left(
        a_{ij}\frac{\partial x_i}{\partial x_k}x_j +
      a_{ij}x_i\frac{\partial x_j}{\partial x_k} \right) \\
      &= \sum_{i = 1}^{n}\sum_{j = 1}^{n} (a_{ij}\delta_{ik}x_j +
      a_{ij}x_i\delta_{jk}) \\
      &= \sum_{j = 1}^{n} a_{kj}x_j + \sum_{i = 1}^{n} a_{ik}x_i \\
      (a_{ik} = a_{ki}\text{より}) &= 2\sum_{i = 1}^{n} a_{ki}x_i
    \end{align*}
  \end{proof}
\end{lemma}
\end{document}
